...
def pick_config(project_dir: Path) -> Dict[str, Any]:
    ...
    user_private_key = cfg.get("QUERY_PRIVATE_KEY", "")
    ...
    return {
        ...
        "attr_count": int(cfg.get("ATTR_COUNT", "8")),
    }
...
def find_solc_binary(project_dir: Path) -> Path:
    ...
    for c in candidates:
        if c.exists():
            return c
    raise RuntimeError("Missing solc-windows.exe in paper-lowlatency-oabe-sepolia/scripts/ or paper-vpre-sepolia/scripts/")
...
def artifact(compiled: Dict[str, Any], source: str, name: str) -> Dict[str, Any]:
    ...
    return {"abi": obj["abi"], "bytecode": obj["evm"]["bytecode"]["object"]}
